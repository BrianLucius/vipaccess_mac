...
- `VIPAccess/App/VIPAccessApp.swift` — `@main` App. Defines the `MenuBarExtra` scene and the floating `Window("VIP Access", id: "token-window")`. Owns the single `@StateObject TokenViewModel`. Loads the credential in the label's `.onAppear` (fires at launch, not just when the menu opens). Shows a critical alert + quits if no credential is found.
...
- `VIPAccess/Core/KeychainCredentialStore.swift` — two-path credential access: read from the app's own login-keychain entry first (`SecItem`, service `com.vipaccess.credential`, separate `id` and `secret` accounts); on miss, migrate from the legacy Symantec keychain and store the result. Also defines `KeychainError` and a base32 `Data` extension.
- `VIPAccess/Core/LegacyKeychainReader.swift` — isolated, one-time migration reader for the Symantec `VIPAccess.keychain-db`. Contains all deprecated `SecKeychain*` usage plus the `/usr/bin/security` CLI fallback and AES-128-CBC decryption. See the gotchas steering file before touching this.
- `VIPAccess/ViewModels/TokenViewModel.swift` — `@MainActor ObservableObject`. Published: `currentCode`, `nextCode`, `secondsRemaining`, `credentialID`, `isLoaded`, `error`. 1-second `Timer` (scheduled on `.common` run loop mode so it keeps firing during menu tracking). `loadCredential()`, `forceRemigrate()`, `startRefresh()`, `refresh()`.
